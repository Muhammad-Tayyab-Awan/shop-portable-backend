...
const getAllOrderLoggedUser = async (req, res) => {
  try {
...
const createOrderLoggedUser = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
      const userId = req.userId;
      let orderInfo = req.body;
      const deliveryAddress = await Address.findById(orderInfo.deliveryAddress);
...
const assignOrderAdmin = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
...
const cancelOrderLoggedUser = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
      const userId = req.userId,
...
const deliverOrderAdDm = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
...
const addOrderItemLoggedUser = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
      const userId = req.userId,
      ...
      const order = await Order.findById(orderId);
      if (order && order.user.toString() === userId) {
        if (order.status === "In Progress" && order.deliveryMan === undefined) {
          ...
          if (orderedItems === newOrderItems.length) {
            ...
            for (const newOrderItem of newOrderItems) {
              if (prevOrderedProducts.includes(newOrderItem.productId)) {
                ...
                const product = await Product.findById(newOrderItem.productId);
                const newItemPrice =
...
const removeOrderItemLoggedUser = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
      const userId = req.userId,
...
const getCanceledOrdersLoggedUser = async (req, res) => {
  try {
    const userId = req.userId;
...
const getDeliveredOrdersLoggedUser = async (req, res) => {
  try {
    const userId = req.userId;
    const orders = await Order.find({
      user: userId,
...
const getOnWayOrdersLoggedUser = async (req, res) => {
  try {
    const userId = req.userId;
...
const getPendingOrdersLoggedUser = async (req, res) => {
  try {
    const userId = req.userId;
    let orders = await Order.find({
      user: userId,
...
const getAllOrdersParticularUserAdmin = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
...
const getAllOrdersParticularDeliveryManAdmin = async (req, res) => {
  try {
    const result = validationResult(req);
    if (result.isEmpty()) {
...
const orderController = {
  getAllOrderLoggedUser: getAllOrderLoggedUser,
  createOrderLoggedUser: createOrderLoggedUser,
  assignOrderAdmin: assignOrderAdmin,
  cancelOrderLoggedUser: cancelOrderLoggedUser,
  cancelOrderAdmin: cancelOrderAdmin,
  deliverOrderAdDm: deliverOrderAdDm,
  addOrderItemLoggedUser: addOrderItemLoggedUser,
  removeOrderItemLoggedUser: removeOrderItemLoggedUser,
  getCanceledOrdersLoggedUser: getCanceledOrdersLoggedUser,
  getDeliveredOrdersLoggedUser: getDeliveredOrdersLoggedUser,
  getOnWayOrdersLoggedUser: getOnWayOrdersLoggedUser,
  getPendingOrdersLoggedUser: getPendingOrdersLoggedUser,
  getAllOrdersAdmin: getAllOrdersAdmin,
  getAllOrdersParticularUserAdmin: getAllOrdersParticularUserAdmin,
  getAllOrdersParticularDeliveryManAdmin: getAllOrdersParticularDeliveryManAdmin
};
